# OVON Cache Prompt Split

These files define the explicit-context-cache prompt split for the main object-navigation / OVON prompt family.

## Design Principles

- `*.system.prompt.md`
  - Stores stable long-form rules, formatting constraints, and examples that benefit from cache reuse.
  - May keep fixed threshold placeholders such as `{obs_blocked_m}`, `{arrival_near_m}`, or `{strict_stop_m}`.
- `*.user.prompt.md`
...
- `planning_initial.system.prompt.md`
  - Stable initial search / planning rules, input legend, output schema, and examples.
