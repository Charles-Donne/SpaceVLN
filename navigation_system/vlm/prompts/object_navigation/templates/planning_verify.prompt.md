...
Analyze each provided IMAGE separately. Use RGB/layout first, then obstacle distances and landmark labels. Distinguish the current space from farther spaces seen through openings. Infer the current standard area type from nearby scene layout/objects/openings; old waypoints/chains are history only. State visible target-object evidence, visible proxy cues, likely connectors, blocked/tight views, and generic/backtracking directions. Conclude with: Current Position Guess | Reachable Far Area/Landmark | Target-Space / Object Direction Guess | Blocked.
...
{{
    ...
    "current_waypoint": "<Write exactly `space area - nearby landmark / nearby landmark / nearby landmark`. The left side must be a standard current area type inferred from current nearby views/layout; use `your current area` only when consistent. Previous Space Waypoints/chains are history only. The right side must contain only nearby current landmarks/cues; if the target object is already beside you and the previous subtask summary shows arrival within about {strict_stop_m}m, include it there too.>",
