...
1. **Current Position**: infer where you are now from the current observations, Space Structure, map, and recent movement evidence. Write `current_waypoint` exactly as `space area - nearby landmark / nearby landmark / nearby landmark`: the left side before `-` must be the current space/area type or name inferred from the current surrounding views, nearby objects, openings, and layout; the right side after `-` must contain only nearby current landmarks/cues. The current space/area must follow the nearby current scene, not the last visited waypoint label: previous Space Waypoints / chains only show where you have been. Use the bottom-strip `your current area` tag only as supporting evidence for the left side when it agrees with RGB/layout. If nearby evidence shows you already crossed into a new room/area, rename the current space to that new concrete space type immediately. If the target object is already beside you and the Previous Subtask landmark summary shows it within about {strict_stop_m}m / already reached, include that target object after `-`.
...
{{
    ...
    "current_waypoint": "<Write exactly `space area - nearby landmark / nearby landmark / nearby landmark`. The left side must be the current space/area type or name inferred from current nearby views, objects, openings, layout, and the preserved `your current area` tag when consistent. Previous Space Waypoints / chains are history only; if the nearby scene shows a new room/area, update the left side to that concrete space/area immediately. The right side must contain only nearby current landmarks/cues; if the target object is already beside you and the previous subtask summary shows arrival within about {strict_stop_m}m, include it there too.>",
