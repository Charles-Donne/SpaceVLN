...
class VLMControllerOptions:
    """Normalized controller runtime options loaded from config.MAP."""
    ...
    def from_map_config(
        ...
        *,
        ...
        return cls(
            enable_auto_retreat=bool(getattr(map_config, "ENABLE_AUTO_RETREAT", False)),
            save_api_request_artifacts=bool(
                getattr(map_config, "SAVE_API_REQUEST_ARTIFACTS", False)
            ),
            save_navigation_step_images=bool(
            ...
            save_waypoint_memory=bool(
                getattr(map_config, "SAVE_WAYPOINT_MEMORY", False)
            ),
            save_best_result_copy=bool(
                getattr(map_config, "SAVE_BEST_RESULT_COPY", False)
            ),
