...
from navigation_system.mapping.mapper import SemanticMapper
from navigation_system.mapping.processor import SemanticProcessor
from navigation_system.mapping.semantic_mapping import Semantic_Mapping
from navigation_system.visualization.visualizer import MapVisualizer
...
from navigation_system.vlm.support.schema import DIRECTION_CONFIG
...
from navigation_system.env import ensure_env_registered
from navigation_system.env.env_utils import construct_envs
    ...
    def __init__(self, config: Config):
        ...
        if env_class is None:
            raise RuntimeError(
                ...
                "Please check navigation_system.env.environments."
            )
        ...
        self.visualizer = MapVisualizer(
            ...
            save_step_map_artifacts=bool(getattr(self.config.MAP, "SAVE_STEP_MAP_ARTIFACTS", False)),
        )
    ...
    def _refresh_post_action_landmark_detection_state(self, action_phase: str) -> bool:
        ...
        _, detected_landmarks_step, _ = self._save_visualization_snapshot(
            ...
            render_policy={
                ...
                "render_local_map": bool(getattr(self.config.MAP, "SAVE_STEP_MAP_ARTIFACTS", False)),
                "save_local_map": bool(getattr(self.config.MAP, "SAVE_STEP_MAP_ARTIFACTS", False)),
