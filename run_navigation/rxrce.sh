#!/bin/bash
# RxR-CE launcher for the shared Navigation Agent.
...
spacevln_navigation_print_usage() {
    cat <<'EOF'
Usage:
  bash run_navigation/rxrce.sh [--runtime standard|context_cache] [--ablation PRESET_OR_YAML] [episode_args...]
...
Episode-arg examples:
  ordered 1 2433 260 skip-sr1 5
  ordered 321 2113 260 skip-sr1 5
  list 6,18,27 260 all 3

Examples:
  bash run_navigation/rxrce.sh --runtime context_cache ordered 1 2433 260 skip-sr1 5 --output-profile debug
  bash run_navigation/rxrce.sh --runtime context_cache ordered 321 2113 260 skip-sr1 5 --output-profile metric
EOF
}
