...
5. **Initial Progress marking**: because this is the task start, nothing is completed yet. Mark only the start/current subtask as `(Current)` and all later stages as unmarked; do not use `(✓)` unless the final target is already reached at the start.
