...
**Conclusion**: Identify from 12-view content: Current Position Guess: [current space + NEAR objects + surrounding scene across adjacent views] | Available Area: [which IMAGEs are safe to other spaces or FAR objects] | Blocked=[which IMAGEs are <0.5m] 
...
5. **Task Progress start**: because this is the task start, nothing is completed yet. Mark only the start subtask as `(Current)` and all later stages as unmarked; do not use `(✓)` unless the final target is already reached at the start.
...
{{
    ...
    "task_progress": "<Initial state: nothing completed yet; only the current/start stage is (Current), all future stages are unmarked>",
...
1. **Identify Current Area**: read the current area from the map and space structure, but always cross-check it with Part 1 nearby evidence. Use the 12 IMAGEs, especially NEAR objects and the surrounding scene continuity across adjacent views, to determine where you are now; if the map current area is `Unknown` or conflicts with the nearby-view evidence, resolve the current space/local place from the strongest consistent 12-view nearby evidence.
...
1. **Current Position**: Localize yourself primarily from the current 12 observations, especially nearby objects, nearby walls/openings/furniture layout, and the surrounding scene consistency across neighboring views. Use map pose and space structure only to support/disambiguate, not as the sole basis. State both the current space and landmarks/local place inside it in the form "[space] - near/by/at [local objects/place]".
2. **Final Task Goal**: know the final task destination is what objects in what space.
3. **Parse Full Task**: break the task into stages from the current start position to the final goal. 
