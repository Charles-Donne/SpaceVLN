...
**Evidence order**: read each IMAGE in this order: NEAR current/large objects + implied space; FAR objects/openings + implied space; obstacle distance + blocked/caution/passable; landmark + shown distance. Judge RGB/layout first and use labels/distances only as support. If a label conflicts with the scene, trust the scene. Compare all 12 directions together to decide where you are now, which directions lead to which spaces/space landmarks, and which view most likely leads toward the next destination. Use layout, openings, furniture relations, and adjacent-view consistency. Use NEAR evidence first to decide the current space type: room interior, hallway, landing, or other transition/outside area. Do not call the current space a room just because that room is visible ahead through an opening; a hallway stays hallway unless nearby evidence places you inside the room. In stair scenes, infer upstairs/downstairs from stair edges, rise/drop trend, landing continuity, railings, adjacent views, and any open no-floor/drop side; infer connector from layout, not landmark detections. Also decide whether you are on the stair run, at the top landing, at the bottom landing, or already off the stairs into the adjacent space.
...
1. **Current Start Position**: localize from Part 1. State both **which space contains you now** and **where inside / outside that space boundary you are now**. Use nearby landmarks, walls/openings, furniture layout, adjacent-view continuity, and reliable visible landmark cues; never from one label/distance alone. Decide the current space from nearby evidence first: use a room name only when nearby evidence places you inside it; otherwise use the transition space, such as hallway/landing, rather than a farther room visible ahead. For `current_waypoint`, use `"[space] - [landmark1 / landmark2 / landmark3]"`, prefer concrete nearby/local cues, and use task wording only when observations truly match. At entrances, first decide which side of the boundary you are on: if still inside a bedroom, use `Bedroom - ...`; if already outside with the bedroom ahead, use `Hallway - bedroom doorway / bedroom entrance side`, not `Bedroom - ...`. Never name the current space from a farther room visible through an opening. The strict current anchor must be inside the old space, at the threshold, or already outside/in the next transition space. In stair scenes, decide top vs bottom from geometry and any open drop side; judge whether you are on the stairs, at the top landing, at the bottom landing, or already off the stairs into the next space, and whether the climb/descent is finished. Prefer stair-specific wording. If the task says to stop outside / at another entrance and views already match that outside anchor, use it. `current_waypoint` must stay the real current anchor and update as surroundings change.
...
4. **Task Progress start**: at task start, nothing is complete unless the global goal is already reached. Write `task_progress` as task-ordered natural-language sub-instructions from the Global Task, comma-separated, not arrows and not Space Waypoint Chain order. Mark only the piece aligned with the strict current anchor `(Current)`; later pieces stay unmarked; do not use `(✓)` unless the goal is already reached at the start. Judge each piece from the strict current anchor versus the ordered stage endpoints. A stage endpoint counts as reached only when the localized space/space-type matches and the destination landmark/local anchor is near within about {arrival_near_m}m, or current views clearly show the required entrance/outside anchor. For stair anchors, generic `stairs` visibility is not enough: require the correct top/bottom/landing or on-stairs/off-stairs state demanded by the task. Keep turn cues inside the piece they serve. If views already show you have exited a source room or reached the next entrance-side anchor, mark that leave/exit piece satisfied immediately; do not route back unless the task explicitly requires re-entry. Once a piece is truly satisfied, move to the next one immediately. Do not let `(Current)` lag behind `current_waypoint` or invent future landmarks/spaces.
5. **Task Waypoint Chain**: output the ordered **task-defined** stage-anchor chain matching those stages and `task_progress`, not the executed Space Waypoint Chain order. Every node, including the current/start node, must stay in full `"[space]'s [landmark]"` form, with the current/start node as `"[space]'s [landmark](Current)"`, followed by later task nodes until Goal. Never output bare `Start`, `Current`, `WP#`, or space-only nodes. Preserve task order and do not skip intermediate space/landmark. Turn cues are route guidance, not standalone chain nodes, unless the task explicitly makes the turned-to place itself the destination anchor. Keep unfinished later nodes within task-supported anchors only. `current_waypoint` and `(Current)` in `waypoint_chain` must refer to the same current anchor, though with different styles. `(Current)` must be the strict current anchor, not an earlier passed or later unreached node. Use the Space Waypoint Chain only as auxiliary evidence for which task node is already reached.
6. **Task Goal Arrival Check**: reason from (a) current localized position, (b) the goal's expected local cues, (c) current 12-view nearby evidence, and (d) the waypoint chain / task progress state. First compare the strict current anchor against the current stage endpoint and advance the task state if that endpoint is already reached; then compare the updated strict current anchor against the final goal anchor. Do not decide arrival from one landmark cue or shown distance alone; compare the full task order with the current multi-view scene. The task goal is the exact target space/place named by the Global Task. If the task names another room of the same type, confirm it is the distinct target space, not the current room's doorway/landmark. Goal arrival requires the correct space/space-type plus the goal landmark/local anchor near within about {arrival_near_m}m, or current views clearly showing the required entrance/outside stop anchor. For stair goals, generic `stairs` detection is not enough: confirm the required top/bottom/landing or on-stairs/off-stairs anchor. If the current place is similar but wrong, do not stop. If the task says to enter or pass through another space first, do not stop early just because a later landmark is visible. If current views already support the exact goal anchor, or the exact task-named outside/entrance endpoint where the task says to stop, stop immediately. Set `global_task_finish=true` only if these cues consistently show the goal `"[space]'s [landmark]"` is already reached within about {arrival_near_m}m or the 12 views clearly show arrival; otherwise false.
...
4. Choose `subtask_landmark` only for that first stage. It is the most useful visible cue for executing the current subtask now. Prefer the **next-stage task-mentioned landmark itself** if it is currently visible. If the task does not name a useful next-stage landmark, infer one from the next-stage destination space + current 12-view evidence and choose a necessary visible concrete cue for that stage. Output the landmark phrase itself, not the full destination string and not a `space's landmark` rewrite: use `bed`, `doorway`, `stairs`, `rug`, `bookshelf`, not `Bedroom's bed` or `bedroom doorway`, unless the task itself names a compound landmark phrase. Do not use a broad space type / room name / generic area label as `subtask_landmark`. If no useful visible concrete cue is needed, use an empty string.
...
{{
    ...
    "current_waypoint": "<Space Waypoint style `[space] - [landmark1 / landmark2 / landmark3]`. Use observed nearby distinctive cues; decide the current space from NEAR evidence first, distinguishing room interior vs hallway/landing/outside side; prefer concrete nearby objects/local place cues over broad room labels; use task wording only when observations truly match; at entrances, name the space you are actually in now, not the farther room visible through the doorway; update the landmark wording as local surroundings change.>",
    ...
    "subtask_landmark": "<One clear visible concrete landmark phrase if useful; otherwise empty. Prefer the next-stage task-mentioned landmark itself if visible; otherwise infer a necessary concrete cue from the next-stage destination space + current views. Output the landmark phrase itself, not a `space's landmark` rewrite like `bedroom doorway`. Never use a broad space type / room name as `subtask_landmark`.>",
...
- **Reasoning discipline**: Part 1 stays evidence-only and ends with Current Position Guess / Reachable Far Area/Landmark / Blocked. Read the real 12-view content first and treat detections as noisy support only. Localize first, then build task-defined stages, `task_progress`, `waypoint_chain`, and Task Goal Arrival Check. Keep same-space pass/through/around inside one stage, split cross-space transitions, preserve active task motion cues, and reject backtracking.
- **Current waypoint naming**: `current_waypoint` must follow actual local evidence, not older waypoints. Use Space Waypoint style `"[space] - [landmark1 / landmark2 / landmark3]"`. Resolve inside/outside and current space type first from NEAR evidence: room interior, hallway, landing, or other transition/outside area. The named space must be the space the agent is actually in now, not an adjacent room merely visible through a doorway. Use task wording only when observations truly match, and keep `current_waypoint` aligned with `(Current)` in `waypoint_chain`.
- **Landmark discipline**: prefer the next-stage task-mentioned landmark itself, or a clearly task-relevant concrete cue inferred from the next-stage destination space + current views. `subtask_landmark` should be the landmark phrase itself, not a `space's landmark` rewrite. Do not use a broad room/space type when a specific object/local cue is available.
...
**Evidence order**: read each IMAGE in this order: NEAR current/large objects + implied space; FAR objects/openings + implied space; obstacle distance + blocked/caution/passable; landmark + shown distance; reachable space waypoint + shown distance if that IMAGE explicitly shows one. Judge RGB/layout first and use labels only as support. If a landmark label conflicts with the scene, trust the scene. Compare all provided directions together to decide where you are now, which directions lead to which spaces/space landmarks, and which view most likely leads toward the next destination. Use layout, openings, furniture relations, adjacent-view consistency, and stair geometry (edges, rise/drop, railings, landing continuity, open drop side) to infer upstairs/downstairs. Use NEAR evidence first to decide the current space type: room interior, hallway, landing, or other transition/outside area. Do not call the current space a room just because that room is visible ahead through an opening; a hallway stays hallway unless nearby evidence places you inside the room. Also decide whether you are on the stair run, at the top landing, at the bottom landing, or already off the stairs into the adjacent space. If a detection conflicts with scene/current-position evidence, treat it as noise.
...
1. **Current Position**: localize from {verify_view_count}-View Analysis, then use task order, the Space Waypoint Chain, and the Previous Subtask destination + landmark final observation only as auxiliary evidence of what was already reached. First check whether the previous destination/landmark was truly reached; if current views/structure disagree, treat it only as overshoot/off-route evidence. Never localize from the previous-subtask landmark final distance alone. If the previous landmark says `(you have arrived now)`, use it only when it supports the **correct task-required space/anchor**. Decide the current space from nearby evidence first: use a room name only when nearby evidence places you inside it; otherwise use the transition space, such as hallway/landing, rather than a farther room visible ahead. For `current_waypoint`, use `"[space] - [landmark1 / landmark2 / landmark3]"`, prefer concrete nearby/local cues, and use task wording only when observations truly match. At entrances, first decide which side of the boundary you are on: if still inside the bedroom, use `Bedroom - ...`; if already outside with the bedroom ahead, use `Hallway - bedroom doorway / bedroom entrance side`, not `Bedroom - ...`. Never name the current space from a farther room visible through an opening. The strict current anchor must be inside the old room, at the threshold, or already outside/in the transition or next space. For stair scenes, decide top vs bottom from geometry and any open drop side; judge whether you are on the stairs, at the top landing, at the bottom landing, or already off the stairs into the next space, and whether the climb/descent is finished. Prefer stair-specific wording. If the task says to stop outside / at another entrance and current views already match that outside anchor, use it. If surroundings change, update the landmark wording. `current_waypoint` must be the strict current anchor, not an earlier passed anchor or future target.
2. **Global Task Goal**: state the goal only as `"[space]'s [landmark]"`. Also state the nearby landmarks/local layout that would confirm true arrival, especially if the final landmark may be partly hidden.
...
4. Write the next subtask instruction for only that nearest unfinished stage. It must match the chosen destination and direction, and the same current -> next step implied by the Space Waypoint Chain and task waypoint chain. Choose the instruction form from the Global Task meaning, not just local visibility: if the current task piece is same-space pass-by / through / around, keep that cue + destination in one instruction and keep the destination as the stage's final landmark; if the task changes to another space, keep the instruction only on the current cross-space stage and leave later-stage targets for later. If the pass-by / enter requirement is already effectively complete and the task has advanced, switch to the next stage's destination instead of repeating the old pass-by instruction. If the task gives explicit motion guidance such as turn left/right, go straight, or walk to the end, preserve that guidance when the current route evidence still supports it. Never instruct the agent toward an already reached current node unless the correct action is to stop there. Prefer one of these concise forms when appropriate:
   - Direct approach / enter: `From [next_waypoint_direction] view, start, [move/enter/approach] toward [destination].`
   - Via visible cue / obstacle bypass: `From [next_waypoint_direction] view, start, [pass/go through/go around/cross] [visible cue], then [enter/approach/continue toward] [destination].`
5. Choose the most relevant visible subtask landmark. Prefer the **next-stage task-mentioned landmark itself** if visible. If the task does not name a useful next-stage landmark, infer one from the next-stage destination space + current views and choose a necessary visible concrete cue for that stage. Output the landmark phrase itself, not the full destination string and not a `space's landmark` rewrite: use `bed`, `doorway`, `stairs`, `rug`, `bookshelf`, not `Bedroom's bed` or `bedroom doorway`, unless the task itself names a compound landmark phrase. Do not use a broad space type / room name / generic area label as `subtask_landmark`. If no useful visible concrete cue is needed, use an empty string.
...
{{
    ...
    "current_waypoint": "<Space Waypoint style `[space] - [landmark1 / landmark2 / landmark3]`. Use observed nearby distinctive cues; decide the current space from NEAR evidence first, distinguishing room interior vs hallway/landing/outside side; prefer concrete nearby objects/local place cues over broad room labels; use task wording only when observations truly match; at entrances, name the space you are actually in now, not the farther room visible through the doorway; update it as current evidence shows you have advanced.>",
...
- **Anchor consistency**: `current_waypoint` must follow current local evidence, not older waypoints. Use Space Waypoint style `"[space] - [landmark1 / landmark2 / landmark3]"`. Resolve inside/outside and current space type first from NEAR evidence: room interior, hallway, landing, or other transition/outside area. The named space must be the space the agent is actually in now, not an adjacent room merely visible through a doorway. Use task wording only when observations truly match, and keep `current_waypoint` aligned with `(Current)` in `waypoint_chain`.
- **Landmark discipline**: prefer the next-stage task-mentioned landmark itself, or a clearly task-relevant concrete cue inferred from the next-stage destination space + current views. `subtask_landmark` should be the landmark phrase itself, not a `space's landmark` rewrite. Do not use a broad room/space type when a specific object/local cue is available.
- **Task chain / stop**: `waypoint_chain` is the task-required anchor order, not the real executed Space Waypoint Chain. Judge `task_progress` before `waypoint_chain`, keep exactly one `(Current)` piece, advance only when the true stage endpoint is reached, and stop only at the exact required target.
