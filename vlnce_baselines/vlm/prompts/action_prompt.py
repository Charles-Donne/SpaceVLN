...
- If this stage is a connector-like route such as doorway / hallway / passage / stairs, analyze current RGB geometry and prefer the visible opening or stair run middle / centerline instead of hugging a side wall, door frame, railing, or corner. For upstairs/downstairs, decide first whether the task needs up or down and keep only that run. If FRONT shows the needed run, or stair edge + rise/drop geometry shows the continuation, treat that short stair-facing distance as stair geometry, not a wall/block test. For downstairs, the needed run may be a partly hidden open drop / missing-floor beyond a stair edge or railing
...
{{
    "reasoning": "One concise chain: environment perception + current view, landmark/space-structure cues, position/progress/arrival check, then the safest action toward the current-stage destination",
...
**Ex1 - Clear path**
{{
    "reasoning": "The heading is aligned, FRONT is open, and the goal is still far and only mildly side-front, so forward beats turning early.",
...
{{
    "reasoning": "FRONT is warning/blocked, right is also warning, and left is the only open/passable side, so detour left and re-center later.",
...
{{
    "reasoning": "The destination is in front and close, but arrival is not yet satisfied, so take a short forward step.",
...
- **Landmark/route validity**: landmark detections are candidate evidence, not ground truth. Validate them against RGB appearance, local geometry, obstacle layout, task destination, and space structure. For doorway / hallway / passage / stairs stages, move through the correct route middle / centerline; do not hug side walls, door frames, or railings, or choose a side branch just because it looks open. For relational instructions, use the named object as reference evidence, not the stop target unless `Destination` itself names it.
- **Stage-following + stop**: treat `Destination` as the current-stage goal and `Instruction` as the route relation. Finish the current enter-stage before any later target. For stairs, follow only the task-required up/down run; for downstairs, a partly hidden descending side still counts when task + geometry support it. If `Destination` is not yet reached, do not output `STOP`; if it is reached, output `STOP` immediately and do not drift past it or stop early at an intermediate cue.
