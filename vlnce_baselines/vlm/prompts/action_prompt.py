...
4. **Action Decision + Obstacle Avoidance**: choose one safe immediate action that follows the subtask and moves toward the destination. Jointly analyze image content, landmark distance/direction, obstacle distance, task instruction, and arrival state. Apply the grouped rules below in order: arrival, then stage-following/direction, then forward-step selection. Keep the final answer inside the fixed action space.
   **Action guidance**:
   a. **Destination-first stage following**: `Destination` is the priority target. Use `Instruction` only to decide whether to go directly or via a cue. Same-space pass/go-by/around keeps the cue as route guidance; cross-space enter stays on the current enter-stage destination. For relational instructions, the named reference object is only a cue unless `Destination` itself names it; if the task says doorway left/right of a staircase, the staircase is reference evidence and the doorway/opening is the target. Always read `Subtask Progress` and `Previous Step Analysis` to judge what is already finished and whether the last turn already aligned the agent. Treat landmark detections as supporting evidence unless the current view and task context also support them. Choose the road/opening that actually leads toward the correct destination, not just the nearest open side or the most obvious reference landmark. If the reference landmark is ahead and the target side-opening is not yet separable, approaching forward can be correct only while that forward route still approaches the target region rather than the reference object itself.
   b. **Turn only when needed**: turn only when the destination or required cue is off-front, FRONT is blocked/tight, recent forward movement failed, or the instruction requires a side entry. If the destination landmark is already near and clearly off-front / beside, turn to face it before forward, but only when it looks visually valid and task-aligned. If a visible landmark is only a reference cue or a pass-by / intermediate cue toward a farther destination, do **not** walk into or turn to center it as if it were the stop target. For doorway / hallway / passage / stairs stages, face the correct route middle / centerline, not a side edge or frame. If the instruction says the destination is left/right of a visible reference object, and FRONT mainly shows that reference object while the target route is already separable or clearly offset to the instructed side, turn/offset toward that side instead of continuing straight into the reference object. If the stage is upstairs/downstairs and FRONT is the required stair run or clearly enters it from stair geometry, do not side-turn just because the stair-facing distance is short; keep advancing along the needed up/down direction. Otherwise, when FRONT is blocked or warning/tight, rule out blocked/tight side routes first, prefer the non-warning open/passable side, and use destination alignment only to break ties between similarly safe sides. If one side is open/passable and the other is warning/blocked, choose the open/passable side even if the landmark lies slightly toward the worse side, then re-center later. If the blocked-front warning is present, treat straight movement into that same FRONT route as forbidden on this call unless FRONT is clearly the required stair run. Do not turn into a wrong-space or backtracking open side. After a detour, re-center once the destination is reachable again.
   c. **Forward-first when already aligned**: thinking usually hands off a destination-aligned heading. If `Subtask Progress` is empty / `Just started` and `Previous Step Analysis` is empty / `N/A (first step)`, treat the current facing as aligned; if FRONT is passable and no stronger evidence puts the destination off-front, prefer forward over exploratory turning. If the destination or most relevant task landmark is clearly and plausibly visible ahead and still far, prefer forward. If FRONT mainly shows a reference landmark while the instructed destination is to its left/right side, forward-first still applies only when the current heading is clearly approaching that relational target region and not merely centering the reference object. If the destination landmark is still far and only mildly off-front, such as in the left/right 30deg side-front sector, prefer forward first and adjust later. More generally, when FRONT is task-aligned, prefer forward; use turns mainly for obstacle avoidance, task-required side entry, or clear off-front destination evidence.
   d. **Forward distance selection**: choose forward distance from the best available target-distance evidence in this order: valid destination detection > valid subtask-landmark detection > bottom-strip landmark/space-waypoint distance > visible free-space depth. Use larger steps for far aligned targets and smaller steps for near targets or tight clearance. If the destination / landmark is visible and still far, keep moving forward. If a near landmark is **not** the destination, keep moving toward the true destination rather than centering the cue. If the destination itself is near and clearly off-front / beside, adjust heading first; if it is far and only slightly side-front, keep forward progress first. If FRONT mainly contains a non-destination reference landmark and the route to the true destination is offset left/right by instruction semantics, adjust first instead of stepping straight into the reference object. If the current heading is already task-aligned, choose forward distance instead of extra turning. For upstairs/downstairs stages, if FRONT is the correct stair run, keep moving along the stair middle / centerline in the needed up/down direction instead of treating that stair-facing distance as ordinary blocking. After a side detour, clear the obstacle with forward moves and turn back once the aligned route reappears.
...
- **Visible-Target Rule**: if the destination or most relevant task landmark is visibly plausible and still far, especially if only mildly side-front, prefer forward. If it is already near and clearly off-front / beside, adjust angle first. If FRONT mainly shows a reference landmark but instruction semantics place the true destination to its side, do not treat that reference object as the forward target. A dubious detection label alone is never enough reason to turn.
...
- **Arrival & Stop**: `Destination` is the exact stop target. If it is not yet reached, do not output `STOP`; if it is reached, output `STOP` immediately. Stop as soon as the exact doorway/opening/place is reached; do not keep drifting past it. Do not stop merely because FRONT is blocked or an intermediate cue/opening appears.
- **Stage-Following & Direction**: treat `Destination` as the current-stage goal and `Instruction` as the route relation. Finish the current enter-stage before any later target. If `Instruction` describes the goal relationally, use the named object as reference evidence and move toward the actual relational target opening/place. For stairs, infer the required up/down run from the current view + stage meaning; for downstairs, choose the descending side when task + geometry support it. Once the correct stair run is identified, prefer its middle / centerline and keep advancing. If FRONT is blocked or warning/tight and is not the correct stair run, prefer a destination-supporting non-warning side over a warning/blocked side.
