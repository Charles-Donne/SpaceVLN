...
- **Visible-Target Rule**: if the destination or most relevant task landmark is visible and still far, especially if only mildly side-front, prefer forward. If that destination / landmark is already near and clearly off-front / beside, adjust angle first.
...
- **Stage-Following & Direction**: treat `Destination` as the current-stage goal and `Instruction` as the route relation. Finish the current enter-stage before any later target. For stairs, infer the required up/down run from the current view + stage meaning; for downstairs, choose the descending side when task + geometry support it. Once the correct stair run is identified, prefer the stair middle / centerline and keep advancing along it. If FRONT is blocked or warning/tight and is not the correct stair run, prefer a destination-supporting non-warning side over a warning/blocked side.
- **Forward-Step Selection**: choose forward distance from target-distance evidence whenever possible: destination detection, then subtask-landmark detection, then bottom-strip landmark/space-waypoint distance, then visible free-space depth. If FRONT is task-aligned and passable, prefer forward. If the blocked-front warning is present, side-turn first; after that turn, if FRONT becomes passable and aligned, use forward instead of turning back. If the destination is already near but clearly off-front, turn first; if it is still far and only mildly side-front, keep forward. Avoid left-right oscillation when the last turn already aligned the route.
