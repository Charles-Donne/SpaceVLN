...
- **Visible-Target Rule**: if the destination or most relevant task landmark is plausibly visible and still far, especially if only mildly side-front, prefer forward. If it is near and clearly off-front, adjust first. If FRONT mainly shows a reference landmark but instruction semantics place the true destination to its side, do not treat that reference object as the forward target. A dubious label alone is never enough reason to turn.
...
- **Arrival & Stop**: `Destination` is the exact stop target. If it is not yet reached, do not output `STOP`; if it is reached, output `STOP` immediately. Stop at the exact doorway/opening/place; do not drift past it. Do not stop merely because FRONT is blocked or an intermediate cue/opening appears.
- **Stage-Following & Direction**: treat `Destination` as the current-stage goal and `Instruction` as the route relation. Finish the current enter-stage before any later target. If `Instruction` is relational, use the named object as reference evidence and move toward the true relational target opening/place. For stairs, infer the required up/down run from the current view + stage meaning; for downstairs, prefer the descending side when task + geometry support it. Once the correct stair run is identified, keep to its middle / centerline. If FRONT is blocked or warning/tight and is not the correct stair run, prefer a destination-supporting non-warning side.
- **Forward-Step Selection**: choose forward distance from target-distance evidence whenever possible: destination detection, then subtask-landmark detection, then bottom-strip landmark/space-waypoint distance, then visible free-space depth. If FRONT is task-aligned and passable, prefer forward. If the blocked-front warning is present, side-turn first; after that turn, if FRONT becomes passable and aligned, go forward instead of turning back. If the destination is near but clearly off-front, turn first; if it is far and only mildly side-front, keep forward. Avoid left-right oscillation when the last turn already aligned the route.
