...
4. **Action Decision + Obstacle Avoidance**: Choose one safe immediate action that matches the subtask instruction and moves toward the destination.
   **Action guidance**:
   a. **Destination-first stage following**: the `Destination` is the priority target for this current stage. Use the instruction style only to decide whether to go directly or via a cue. If the current stage is same-space pass/go-by/around, treat the cue as route guidance and keep moving toward the destination. If the current stage is cross-space enter, keep the action on entering/approaching that current stage destination, not a later-stage target.
...
**Critical Rules**:
- if current evidence satisfies the arrival rule for the actual current subtask destination, output `STOP` immediately; otherwise do not use `STOP` merely because FRONT is blocked, the view is narrow, an intermediate cue is nearby, or a pass-by / opening cue has appeared. The system also auto-ends the subtask when a highest-confidence top-2 destination landmark is close enough: about 0.75m for solid landmarks, about 0.5m for openings, or already >90deg / behind for passed-through openings
...
- use one common space type only; ignore modifiers and normalize corridor-like wording to `hallway`
...
- treat `Destination` as the actual goal of the current stage and `Instruction` as the route relation for reaching it: direct enter/approach stays on the destination route; pass/go through/go around/cross stays within the same current task piece and moves through the cue toward the destination rather than stopping at the cue; if the task changes space, finish the current enter-stage first and do not jump to a later-stage target
...
- always use current visual evidence together with the subtask destination, subtask instruction, visible space waypoint cues, and space structure to move toward the correct next task space/landmark
- if a pass-by / opening cue is already beside or behind but the current-stage destination is still ahead, keep centering actions on the destination rather than turning back toward the old cue
