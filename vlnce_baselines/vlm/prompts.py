...
{{
    ...
    "reasoning": "<Max 250 words: CRITICAL: FIRST determine current position, THEN mark waypoints/tasks based on TRUE position. 1) Current Position Analysis (MOST IMPORTANT): WHERE AM I ACTUALLY? Analyze ALL 12 views - what's NEAR (< 1.0m, large, occupying view) vs FAR (small/distant, visible but not reached)? Current position = space defined by NEAR objects. Global Map: which room is red arrow in? Historical waypoints (blue circles) positions? Synthesis: Current waypoint = TRUE position (NOT distant objects I see). CRITICAL: Seeing destination ≠ At destination! If current position matches a previous waypoint (blue circle), you went BACKWARDS - must ROLLBACK markers! 2) Task Chain Position (Based on Step 1): Based on TRUE current position, where in task chain? Which waypoints truly passed (✓)? Which is current (Current)? Which are future (unmarked)? Task progress must align with waypoint status. 3) Next Waypoint & Distance Analysis (CRITICAL): What's the next immediate waypoint? In which direction (IMAGE 1-12)? How far away (obstacle distance)? Can I get closer? List key directions with distances. If destination FAR (>2m, small in view) → continue approaching. If destination NEAR (<2m, occupying significant view area) AND cannot get closer (front blocked or <0.5m clearance) → arrived, can proceed to next. Try to get as close as possible, ideally <0.5m. Avoid: backwards (blue circles), walls (<0.5m). 4) Near-term Plan: System rotates to chosen direction. Explain subtask. 5) Long-term Plan: Remaining waypoints to goal?>"
}}
...
- **Accurate Position Awareness (CRITICAL - Seeing ≠ Arriving)**: Current position = space defined by NEAR objects (< 1.0m, large in views). Arrival judgment: 1) Seeing destination FAR (>2m, small in view) → NOT arrived, continue. 2) Seeing destination NEAR (<2m, occupying significant view area 充斥画面) → check if can get closer. 3) At destination: <2m + destination filling view + cannot advance further (blocked or <0.5m clearance ahead). Try to get as close as possible, ideally <0.5m, but acceptable if cannot get closer.
...
- **Next Waypoint Distance Check (CRITICAL)**: Before choosing direction, analyze: Where is next waypoint? Which IMAGE (1-12)? How far away (obstacle distance)? Can I get closer? Decision logic: 1) If destination >2m away OR small in view → continue approaching. 2) If destination <2m AND occupying significant view area (充斥画面) → check if can get closer. 3) If cannot get closer (front blocked, <0.5m clearance, or already very close) → arrived, proceed to next. Goal: Get as close as possible, ideally <0.5m, but acceptable if <2m with destination filling view and path blocked.
...
- **Global Task Completion (CRITICAL - Get As Close As Possible!)**: Set global_task_finish=true when ALL conditions met: 1) ALL task stages marked (✓), no (Current) remaining, 2) Final destination visible and close: distance <2m AND destination occupying significant view area (充斥整个画面), 3) Cannot get closer: front direction shows obstacle distance <0.5m (blocked) OR already at closest point. Try to get <0.5m if possible, but acceptable if <2m with destination filling view and cannot advance further. DON'T stop if: destination >2m away OR small in view (continue!), can still move 0.25m closer safely (distance >0.5m ahead).
...
{{
    ...
    "completion_criteria": "<Detection: what detected + distance | Location: position/area | Map: space + trajectory>",
    "global_task_finish": <true ONLY when ALL task stages are completed (all marked with ✓, no (Current) remaining) AND you are at the final destination. false otherwise>,
    "reasoning": "<MAX 250 words: CRITICAL: FIRST determine current position, THEN mark waypoints/tasks. 1) Current Position Analysis (MOST IMPORTANT): WHERE AM I ACTUALLY? List observations for ALL 12 views - what's NEAR (< 1.0m, large, occupying view) vs FAR (small/distant, visible but not reached)? Current position = space with NEAR objects. Global Map: red arrow in which room? Historical waypoints (blue circles) positions? If current position matches previous waypoint, YOU WENT BACKWARDS - ROLLBACK! Synthesis: Current waypoint = TRUE position (NOT distant objects I see). CRITICAL: Seeing ≠ At destination! 2) Task Chain Position (Based on TRUE position): Which waypoints passed (✓)? Current (Current)? Future (unmarked)? If at previous waypoint, ROLLBACK markers. Task progress must align. Only ONE (Current). If all (✓) with no (Current), check if truly at final destination! 3) Next Waypoint & Distance Analysis (CRITICAL): What's next immediate waypoint? In which direction (IMAGE 1-12)? How far (obstacle distance)? Can I get closer? List key directions with distances. If destination >2m away OR small in view → continue. If destination <2m AND occupying significant view area (充斥画面) → check if can approach further. If cannot get closer (blocked, <0.5m clearance) → arrived. Try to get as close as possible. Eliminate: walls <0.5m, backwards (blue circles). 4) Near-term Plan: Rotate to chosen direction. Subtask after rotation. 5) Long-term Plan: Remaining waypoints?>"
}}
...
- **Global Task Completion (CRITICAL - Get As Close As Possible!)**: Set global_task_finish=true when: 1) ALL task stages (✓), no (Current), 2) Final destination close: distance <2m AND occupying significant view (充斥画面, large in view), 3) Cannot approach further: front obstacle distance <0.5m (blocked) OR already at closest safe point. Ideal distance <0.5m, acceptable if <2m with destination filling view and path blocked. DON'T stop if: destination >2m away OR small in view (continue!), can still advance 0.25m safely (distance >0.5m ahead).
