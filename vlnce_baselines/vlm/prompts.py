...
**Initial state**: You are at the task start and no progress has been made yet. Follow the instruction from the beginning, first complete the first part of the first sentence, and reach that first subtask destination.
...
**Room/space types**: Use one common room/space type from {common_space_types}; remove modifiers and keep one type only.
...
**Current mapped area**: `Current Area` is pose-based; `Unknown` means outside mapped areas.
...
- **Output constraints**: `current_waypoint`, `next_waypoint_destination`, and all room/space references must use one common room/space type from {common_space_types}; remove modifiers and treat corridor-like wording as `hallway`. `next_waypoint_landmark` must be a clear recognizable object/furniture phrase, not door/doorway/hallway/corridor. `next_waypoint_destination` must stay in "[room]'s [object]" form, and detail phrases should use [room]+[relation]+[object], e.g. "Living room's gray couch", not "couch". `subtask_instruction` must be one short immediate sentence for only the nearest unfinished room/object subtask, in the fixed form "From [next_waypoint_direction] view, start, [action + optional pass-by/path cue + destination]." Use direct verbs such as move/go/walk/enter/pass/follow/cross/approach/continue/head/climb/ascend/descend/stop. The action module automatically drops the "From ... view, start," prefix.
