...
- **12-Direction Analysis (CRITICAL for Part 3)**: In reasoning Part 3, analyze EACH direction (IMAGE 1-12): what's visible? obstacle distance? List key directions considered. Avoid backwards/walls.
- **Direction Selection Logic**: Choose next_waypoint_direction where: 1) Analyze ALL 12 views' content and obstacle distances, 2) Avoid backwards (toward blue circle waypoints), 3) Avoid walls/obstacles (< 0.5m), 4) Choose where destination visible/likely AND distance > 0.5m AND forward progress.
...
- **Direction Selection (CRITICAL for Part 3)**: In reasoning Part 3, analyze EACH direction (IMAGE 1-12): what's visible in each view? obstacle distance for each? List key directions considered with reasoning. Avoid: blue circle waypoints (backwards), walls (< 0.5m), orange trajectory areas (revisit). Choose: destination visible/likely, distance > 0.5m, forward progress toward goal.
