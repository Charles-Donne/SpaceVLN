...
{{
    ...
    "reasoning": "<Max 250 words: CRITICAL: Base ALL reasoning on actual observations from 12 views and maps. Maintain logical consistency. 1) Current Position & Waypoint Analysis: WHERE AM I ACTUALLY? Analyze ALL 12 views - identify what's in each direction, NEAR objects (< 1.0m, large in views) vs FAR objects (small/distant). Analyze Global Map: check current space via trajectory (which space is red arrow in? did trajectory enter new space?), historical waypoints positions/directions. Local Map: green circle (0.5m radius) immediate surroundings. Synthesis: Current waypoint = which SPACE? 2) Task Chain Position & Spatial Relationships: Based on current position, where in task chain? Analyze Global Map: historical waypoints (blue circles if any) in which directions? Trajectory shows which waypoints passed? Spatial perception between spaces. Determine: waypoints completed (✓), current waypoint (Current), future (unmarked). Task progress alignment - stages completed (✓), current stage (Current), future (unmarked). 3) Next Waypoint & Direction OR Final Destination: If NOT at destination - what's next waypoint? In which IMAGE (1-12) most centered? Verify obstacle distance > 0.5m, map safe path. If AT final destination (landmarks VERY NEAR < 0.5m OR at entrance with new space ahead + trajectory shows arrived), stop immediately (global_task_finish=true). 4) Near-term Plan: System AUTO-ROTATES to next_waypoint_direction. Explain subtask AFTER rotation - assume facing waypoint in Front, describe movement. 5) Long-term Plan: Remaining waypoints to complete global task?>"
}}
...
{{
    ...
    "global_task_finish": false,
    "reasoning": "1) Current Position & Waypoint Analysis: Analyze 12 views - IMAGE 4 (Left 90°) and surrounding views show bedroom walls/furniture NEAR (< 1.0m, large in views, defining current space). IMAGE 2 (Left 30°) shows bedroom exit doorway with corridor FAR (pictures visible beyond doorway). Global Map analysis: Red arrow inside bedroom space (enclosed green area), corridor green area extends to the left. Local Map: Green circle shows bedroom interior. Synthesis: Current waypoint = Bedroom (starting position, near exit). 2) Task Chain Position: waypoint_sequence shows Bedroom(Current) - starting point, no waypoints completed. Task progress: No stages completed yet. Current stage: 'Exit the room'(Current - about to execute). Remaining: turn left → head to kitchen → turn right → go through kitchen → exit → reach bathroom. 3) Next Waypoint & Direction: Next immediate waypoint = Corridor with pictures. IMAGE 2 (Left 30°) shows corridor doorway most centered with pictures visible beyond. Verify: obstacle distance > 0.5m (safe), map shows safe path to corridor. Not at final destination (bathroom far away). 4) Near-term Plan: System rotates to IMAGE 2 (Left 30°). After rotation, corridor doorway will be Front. Subtask: Move forward through doorway to reach corridor. Use picture as landmark. 5) Long-term Plan: Enter corridor → turn left → navigate to kitchen → turn right into kitchen → walk through kitchen → exit kitchen → reach bathroom (final goal)."
}}
...
{{
    ...
    "completion_criteria": "<Detection: what detected + distance | Location: position/area | Map: space + trajectory>",
    "global_task_finish": <true ONLY when ALL task stages are completed (all marked with ✓, no (Current) remaining) AND you are at the final destination. false otherwise>,
    "reasoning": "<MAX 250 words: CRITICAL: Base ALL reasoning on actual visual observations - maintain logical consistency. 1) Current Position & Waypoint Analysis: WHERE AM I ACTUALLY? Analyze ALL 12 views - list what's in each direction (Front/Left/Right/Back), identify NEAR objects (< 1.0m, large in views) vs FAR objects (small/distant). Analyze Global Map: check current space via trajectory (which space is red arrow in? did trajectory enter new space?), historical waypoints (blue circles) positions/directions, spatial relationships between spaces. Local Map: green circle (0.5m radius) shows immediate surroundings. Synthesis: Current waypoint = which SPACE (use space name: Bedroom, Hallway, Living Room, etc.)? 2) Task Chain Position & Progress Alignment: Based on current position, where in task chain? Which waypoints passed (✓)? Which is current (Current)? Which are future (unmarked)? Task progress alignment: which stages completed (✓), current stage (Current), future stages (unmarked)? CRITICAL: (Current) marks the stage you are EXECUTING NOW - only one (Current) in task_progress. If all stages are (✓) with no (Current), task is complete! 3) Next Waypoint & Direction OR Final Destination Check: If NOT at final destination yet - what is next immediate waypoint? In which IMAGE (1-12) most centered/visible? Verify: obstacle distance > 0.5m, map safe path. If AT final destination AND all task stages completed (all ✓) - stop immediately (global_task_finish=true). 4) Near-term Plan: System AUTO-ROTATES to next_waypoint_direction. Explain subtask AFTER rotation - assume facing waypoint in Front. 5) Long-term Plan: Remaining waypoints to complete global task?>"
}}
...
{{
    ...
    "global_task_finish": false,
    "reasoning": "1) Current Position & Waypoint Analysis: Analyze 12 views - IMAGE 1 (Front), IMAGE 2-6 (Left sides), IMAGE 8-12 (Right sides) show gym equipment NEAR (< 1.0m, large, occupying significant view area). IMAGE 7 (Back 180°) shows restroom FAR (small/distant, separate space). Global Map analysis: Red arrow inside exercise room space (green area), trajectory shows moved from restroom into exercise room. Historical waypoint (blue circle #1) at restroom position behind. Local Map: Green circle shows gym equipment inside 0.5m. Synthesis: Current waypoint = Exercise Room interior (Waypoint #2). 2) Task Chain Position: waypoint_sequence: Restroom(✓ passed) → Exercise Room(Current = inside now) → Living Room → Table(Goal). Task progress alignment: 'Turn around'(✓ completed), 'walk through exercise room'(Current - executing now, in middle of room), future: 'into living room. Wait by Table'. One (Current) marker for current stage. 3) Next Waypoint & Direction: Next immediate waypoint = Living Room. IMAGE 1 (Front 0°) shows open path through exercise room toward living room entrance - most centered. Verify: obstacle distance > 0.5m (clear), map shows green path ahead to living room. Not at final destination (table not visible, still need to reach living room then table). 4) Near-term Plan: System rotates to IMAGE 1 (already aligned). After rotation, living room path will be Front. Subtask: Move forward through exercise room to reach living room entrance. Use arched doorway as landmark. 5) Long-term Plan: Reach living room entrance → enter living room → locate table → navigate to table → stop at table (final goal). 2 waypoints remaining."
}}
