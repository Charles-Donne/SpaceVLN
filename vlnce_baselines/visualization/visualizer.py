...
class MapVisualizer:
    ...
    def render_global_map(self,
                         ...
                         calculate_waypoint_angle: bool = False) -> Tuple[np.ndarray, np.ndarray, List, np.ndarray, Dict[str, str], Optional[float]]:
        """
        ...
        Args:
            ...
            calculate_distances: 是否计算障碍物距离（环视时设为False加快速度）
            calculate_waypoint_angle: 是否计算最后waypoint角度（只在环视最后一步计算）
        ...
        Returns:
            ...
            - last_waypoint_angle: 最后一个waypoint相对于正前方的角度（弧度），None表示未计算或无waypoint
        ...
        if current_pose is not None:
            ...
            if waypoint_positions and waypoint_ids and len(waypoint_positions) == len(waypoint_ids):
                for idx, ((wp_x, wp_y), wp_id) in enumerate(zip(waypoint_positions, waypoint_ids)):
                    ...
                    # 只在需要时计算最后一个waypoint的角度（相对于正前方）
                    if calculate_waypoint_angle and idx == len(waypoint_positions) - 1:
    ...
    def save_step_visualization(self, 
                               ...
                               controller = None,
                               calculate_distances: bool = False,
                               calculate_waypoint_angle: bool = False) -> Tuple[Dict[str, str], List, Dict[str, str], Optional[float]]:  # 兼容旧参数
        """
        ...
        _, global_map_with_trajectory, landmarks, global_map_clean, obstacle_distances, last_waypoint_angle = self.render_global_map(
            ...
            waypoint_positions, waypoint_ids, calculate_distances, calculate_waypoint_angle
        )
